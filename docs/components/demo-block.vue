<template>
  <div class="demo-block">
    ...
    <div class="demo-block-source">
      ...
      <div class="demo-block-code-icon" :class="{'show':showCode}"  @click="_showcode" ref="codeIcon">
        <span></span><i>{{showCode ? "隐藏代码" : "显示代码"}}</i>
      </div>
    </div>
    <CollapseTransition >
      <div class="demo-block-code"
           v-show="showCode" ref="codeBlock">
...
export default {
  ...
  methods: {
   ...
   this.$nextTick(()=> {
     this.codeHeight =codeBlock.style.height
    //  this.codeHeight =  this.codeHeight ===0 ? 0 : this.codeHeight+30
    //  console.log(this.codeHeight);
    //  console.log( this.$refs.codeBlock.clientHeight);
    this.$refs.codeIcon.style.transform=`translateY(${this.codeHeight})`
   })
    },
...
<style>
  .demo-block{border: 1px solid #ebedf0;box-shadow: 0 0 8px 0 rgba(232,237,250,.6), 0 2px 4px 0 rgba(232,237,250,.5); border-radius: 2px;display: inline-block;width: 100%;position: relative;margin: 0 0 16px;-webkit-transition: all 0.2s;transition: all 0.2s;}
  .demo-block p{padding: 0;margin: 0;}
  .demo-block-code-icon{transition: transform 0.3s ease-in-out; height: 30px;line-height: 30px;border-top: 1px solid #ebedf0;text-align: center;cursor: pointer}
  ...
  .demo-block-code-icon.show span:after{transform: rotate(135deg);margin-top: 16px;transform-origin:  center 30%;}
  ...
  .demo-block .demo-block-code{background-color: #f7f7f7;}
